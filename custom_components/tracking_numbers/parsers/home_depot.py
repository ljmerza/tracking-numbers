...
def parse_home_depot(email):
    ...
    for element in soup.find_all('a'):
        ...
        for candidate in _extract_tracking_candidates(anchor_text):
    ...
    for label in soup.find_all(string=TRACKING_LABEL_RE):
        ...
        if container_text:
            for candidate in _extract_tracking_candidates(container_text):
                _add_entry(candidate)

    # Fallback to whole-body text scraping.
    for candidate in _extract_tracking_candidates(unified_text):
        _add_entry(candidate)
...
def _extract_tracking_candidates(text: str) -> Iterable[str]:
    if not text:
        return ()
    matches: list[str] = []
    tokens = text.split()
